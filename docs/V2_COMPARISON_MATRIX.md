...
- **Notes:** Authoritative C run. Avoid stale `runs/variant_C/` (checkpoint/results mismatch).
  C's 1.5% earlier result was a stale checkpoint bug — see `docs/variant_c_eval_bug.md`.
...
4. Write a DEC-0XX decision record in `docs/DECISION_LOG.md` covering the gate condition and result.
...
`runs/variant_X/` path, which may be stale. See `docs/runs_audit.md` for the
full stale-checkpoint history and `docs/variant_c_eval_bug.md` for the C case study.

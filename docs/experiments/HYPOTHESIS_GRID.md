...
| H4 | Does model size explain the perplexity null? | ◐ | Model size explains the perplexity null | DEC-027: ruled out at 358M (B-small ties A). **Open at larger scale** — but A-large requires a larger-vocab IR candidate; not testable with v2's 601-token design (601 tokens can't meaningfully use 494M+ params). | A larger-vocab IR candidate trained at 494M+; blocked on both a new IR design and compute. |
...
IR/BPE × small/large model — see [`V2_COMPARISON_MATRIX.md`](V2_COMPARISON_MATRIX.md)

...
| H6 | Does the IR improve retrieval / similarity tasks (semantic, not predictive)? | ◐ | The IR improves retrieval / similarity tasks (semantic, not predictive) | Mode 1 (eval→train) clean run (2026-08-18): A_v3_2ep Recall@10=0.0052 vs B_small_clean Recall@10=0.0049. A numerically ahead but CIs fully overlap — not a clean positive. Mode 2 (train→train) not yet run against clean embeddings. Gates: 57/57 invariants pass, all embedding shapes match (train=3375, eval=376). Results: `runs/h6_retrieval/results_clean.json`. See `H6_RESULTS.md` for full table. **Cannot close until Mode 2 confirms direction.** | Mode 2 clean run (train→train against B_small_clean embeddings). If Mode 1 and Mode 2 agree A≥B_small_clean, close ◐ positive (weak). If they disagree, result is ambiguous. |

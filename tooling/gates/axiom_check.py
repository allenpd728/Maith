#!/usr/bin/env python3
"""Gate 6 (Tier 2) — axiom-tracking hygiene for Maith Lean proofs.
...
This is the LOCAL AGENT'S job: it requires the Lean toolchain
(`lake` + `lake env lean`). Intended for CI — after `lake build`, run this so
"no smuggled sorries" is a continuous machine guarantee. Maith has no frozen
theorem targets yet, so this is staged for when the axiom-discovery track
(`docs/experiments/AXIOM_DISCOVERY.md`) produces its first kernel-checked
transferred theorems.

Usage (from the repo root — the lake root):
...
where <import-spec> is a comma-separated list of Lean modules to import, e.g.:
    ...
    python3 tooling/gates/axiom_check.py Maith.Encoder Maith.Encoder.some_key_lemma
